...
public interface UserDao extends JpaRepository<User,Integer> ,JpaSpecificationExecutor {
    ...
    @Query(value = "select u.id, u.username , u.icon from cczu_forum_user u where DATE_SUB(CURDATE(), INTERVAL 30 DAY) <=DATE(u.init_time) ORDER BY u.id DESC limit 12" ,nativeQuery = true)
